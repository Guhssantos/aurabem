...
# Logger de depuração inicial (pode ser mantido ou removido se o problema do arquivo estiver 100% resolvido)
debug_logger = logging.getLogger("startup_debug")
# ... (configuração do debug_logger como antes, se desejar mantê-lo) ...
...
# --- Constantes ---
SESSION_MESSAGES_KEY = "aura_bem_messages" # Nome atualizado
SESSION_CHAT_KEY = "aura_bem_chat_session" # Nome atualizado
SYSTEM_PROMPT_FILENAME = "system_prompt_aura_bem.txt" # Nome do arquivo atualizado
DEFAULT_SYSTEM_INSTRUCTION = ( # Instrução de fallback atualizada
...
st.set_page_config(
    page_title="Aura Bem - Sua Companheira de Bem-Estar", # Título atualizado
    page_icon="💖",
    layout="centered",
...
st.title("💖 Aura Bem: Sua Companheira de Bem-Estar") # Título atualizado
st.caption(
...
system_instruction_aura_bem = "" # Variável atualizada
critical_error_loading_prompt = False

# (Lógica de carregamento do prompt e API Key permanece a mesma,
# apenas substitua 'system_instruction_aura' por 'system_instruction_aura_bem'
# e 'SYSTEM_PROMPT_FILE' por 'SYSTEM_PROMPT_FILENAME' se você usou essas variáveis diretamente)
...
try:
    if os.path.exists(ABSOLUTE_PATH_TO_PROMPT_FILE):
        ...
        if not system_instruction_aura_bem.strip():
            logger.warning(f"Arquivo '{SYSTEM_PROMPT_FILENAME}' vazio. Usando fallback.")
        ...
        st.error(
            ...
            f"não foi encontrado em '{ABSOLUTE_PATH_TO_PROMPT_FILE}'. "
            "Aura Bem usará uma configuração básica. Verifique o deployment."
        )
        system_instruction_aura_bem = DEFAULT_SYSTEM_INSTRUCTION
...
try:
    ...
    logger.info("Chave API do Google configurada.")
except KeyError:
    logger.critical("Chave API do Google NÃO encontrada nos Secrets.")
    st.error("ERRO GRAVE: Chave API do Google não configurada nos 'Secrets'. O app não pode se conectar à IA.")
    st.stop()
except Exception as e:
    logger.critical(f"Erro grave ao configurar API Key: {e}", exc_info=True)
    st.error(f"Erro grave ao configurar API Key: {e}. O app não pode continuar.")
    st.stop()


# --- Bloco 4: Configuração do Modelo Gemini (sem grandes alterações, talvez 'temperature') ---
generation_config = {
    "temperature": 0.65, # Um pouco mais baixa para respostas mais focadas e menos "criativas"
    "top_p": 0.95,
    "top_k": 40,
    "max_output_tokens": 1024, # Um pouco mais de espaço para respostas elaboradas
}
safety_settings = [
...
# --- Bloco 6: Definições de Segurança (CVV) e Detecção de Risco (sem alterações) ---
...
def init_model(instruction: str):
    ...
    try:
        model_instance = genai.GenerativeModel(
            model_name="gemini-1.5-flash-latest", # Ou gemini-1.0-pro se preferir mais robustez em detrimento de velocidade
...
if SESSION_MESSAGES_KEY in st.session_state and len(st.session_state[SESSION_MESSAGES_KEY]) > 1:
    if st.sidebar.button("🧹 Limpar Conversa Atual"):
        ...
        logger.info("Histórico da conversa e sessão do Gemini reiniciados.")
        st.rerun()
...
# --- Bloco 9: Exibição do Histórico (sem alterações) ---
for message in st.session_state[SESSION_MESSAGES_KEY]:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

# --- Bloco 10: Função para Enviar Mensagem e Processar Resposta (lógica interna sem grandes alterações) ---
def send_message_to_aura_bem(user_prompt: str): # Nome da função atualizado
    ...
    try:
        with st.chat_message("assistant"):
            ...
            if SESSION_CHAT_KEY not in st.session_state:
                ...
                logger.info(f"Nova sessão de chat Aura Bem iniciada com {len(history_for_model)} msgs.")

            with st.spinner("Aura Bem está refletindo... 🤔"): # Spinner atualizado
                response_stream = st.session_state[SESSION_CHAT_KEY].send_message(user_prompt, stream=True)

            last_chunk = None
            for chunk in response_stream: # Lógica de streaming e tratamento de erro/bloqueio permanece
                last_chunk = chunk
                # ... (mesma lógica de streaming e tratamento de chunk.parts, chunk.text, prompt_feedback) ...
                if chunk.parts:
                ...
                if chunk.prompt_feedback and chunk.prompt_feedback.block_reason:
                    logger.warning(f"Resposta Aura Bem bloqueada: {chunk.prompt_feedback.block_reason}")
            ...
            if is_blocked_final and not bot_response_final:
                ...
                logger.error(f"Prompt para Aura Bem bloqueado: {reason}")
                ...
                logger.warning(f"Resposta vazia da Aura Bem para: '{user_prompt}'.")
...
if prompt := st.chat_input("Como você está se sentindo hoje?..."): # Placeholder do input atualizado
    logger.info(f"Usuário (Aura Bem): '{prompt[:50]}...'")
    if any(regex.search(prompt) for regex in keywords_risco_regex): # Detecção de risco permanece
        logger.warning(f"RISCO DETECTADO (Aura Bem): '{prompt}'")
        ...
        send_message_to_aura_bem(prompt) # Chamada da função atualizada

# --- Bloco 12: Coleta de Feedback (REMOVIDO) ---
# Esta seção foi removida conforme solicitado.

# --- Bloco 13: Recursos Adicionais (Sidebar) ---
with st.sidebar:
    st.markdown("---")
    st.subheader("Apoio Adicional 🌻") # Título atualizado
    with st.expander("Quando buscar ajuda profissional:", expanded=True): # Expandido por padrão
        st.markdown(
    ...
    with st.expander("Contato de Emergência (Brasil):", expanded=True): # Expandido por padrão
